...
public class ExcelRead {
	...
	public List<ArrayList<String>> readExcel(MultipartFile file)
			throws IOException {
		if (file == null
				|| Contants.EMPTY.equals(file.getOriginalFilename().trim())) {
			return null;
		} else {
			String postfix = ExcelUtils.getPostfix(file.getOriginalFilename());
			if (!Contants.EMPTY.equals(postfix)) {
				if (Contants.OFFICE_EXCEL_2003_POSTFIX.equals(postfix)) {
					return readXls(file);
				} else if (Contants.OFFICE_EXCEL_2010_POSTFIX.equals(postfix)) {
	...
	public List<ArrayList<String>> readXlsx(MultipartFile file) {
		...
		try {
			...
			// ByteArrayInputStream(IOUtils.toByteArray(input));
			// 创建文档
